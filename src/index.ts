...
import {
  detectVendor as registryDetectVendor,
  lookup as registryLookup,
  type OuiRegistry,
 ...
 */
export const KNOWN_LORAWAN_VENDORS: Record<string, string> = {
  A84041: 'dragino',
  '24E124': 'milesight',
  AC1F09: 'rak',
  '2CF7F1': 'seeed',
 ...
 */
export const detectVendor = (devEui: string, registry?: OuiRegistry): VendorInfo | undefined => {
  const match = registry ? registryLookup(registry, devEui) : registryDetectVendor(devEui);
 ...
 * ```
 */
export const parse = (qr: string, opts: ParseOptions = {}): QrParseResult => {
  if (typeof qr !== 'string' || qr.length === 0) {
    throw new QrParseError(String(qr ?? ''), []);
  }
  const raw = qr.trim();
  const attempted: ParseSource[] = [];
 ...
 * ```
 */
export const createParser = (opts: ParseOptions): ((qr: string) => QrParseResult) => {
  return (qr: string) => parse(qr, opts);
};
...
const parseTr005 = (raw: string): Partial<QrParseResult> | null => {
  const m = TR005_RE.exec(raw);
  if (!m) return null;
  const out: Partial<QrParseResult> = {
  ...
  if (m[4]) {
    ...
    for (const field of rest) {
      ...
      if (pos === 0) out.ownerToken = field || undefined;
      else if (pos === 1) out.serialNumber = field || undefined;
      pos++;
    }
  }
  return out;
...
const parseLwdp = (raw: string): Partial<QrParseResult> | null => {
...
const parseJson = (raw: string): Partial<QrParseResult> | null => {
  if (!raw.startsWith('{')) return null;
  let obj: unknown;
  try { obj = JSON.parse(raw); }
  catch { return null; }
  if (!obj || typeof obj !== 'object') return null;
  return extractKeyValues(obj as Record<string, unknown>);
};
...
const KEY_ALIASES: Record<string, keyof QrParseResult> = {
...
const parseKeyValue = (raw: string): Partial<QrParseResult> | null => {
...
const extractKeyValues = (obj: Record<string, unknown>): Partial<QrParseResult> | null => {
  const out: Partial<QrParseResult> = {};
  for (const [k, v] of Object.entries(obj)) {
    if (typeof v !== 'string') continue;
    const field = KEY_ALIASES[k.toLowerCase().replace(/[\s]/g, '')];
    if (!field) continue;
    (out as Record<string, string>)[field] = v.trim().replace(/[\s:-]/g, '').toUpperCase();
  }
 ...
 */
const parseHexScan = (raw: string, opts?: ParseOptions): Partial<QrParseResult> | null => {
  ...
  const eui: string[] = upper.match(/(?<![0-9A-F])[0-9A-F]{16}(?![0-9A-F])/g) ?? [];
  const key: string[] = upper.match(/(?<![0-9A-F])[0-9A-F]{32}(?![0-9A-F])/g) ?? [];
  ...
  if (eui.length === 0 && key.length > 0) {
    for (let i = 0; i < key.length; i++) {
      const k = key[i];
      if (detectVendor(k.slice(0, 16), opts?.ouiRegistry)) {
  ...
  if (eui.length === 0 && key.length === 0) {
    ...
    for (const run of condensed.match(/[0-9A-F]{16,}/g) ?? []) {
      let i = 0;
      while (i < run.length) {
        if (i + 32 <= run.length) { key.push(run.slice(i, i + 32)); i += 32; }
        else if (i + 16 <= run.length) { eui.push(run.slice(i, i + 16)); i += 16; }
        else break;
      }
    }
  }

  if (eui.length === 0) return null;
  ...
  const isHexIdentifier = (t: string): boolean => {
  ...
  for (const c of candidates) {
    if (!serialNumber || c.length > serialNumber.length) serialNumber = c;
  }
  ...
  let bestScore = -1;
  for (const e of eui) {
    const v = detectVendor(e, opts?.ouiRegistry);
    const score = v ? (v.knownLorawanVendor ? 2 : 1) : 0;
    if (score > bestScore) { bestScore = score; devEui = e; }
  }
  const out: Partial<QrParseResult> = { devEui };
  // Remaining 16-hex → joinEui (first one).
  const joinEui = eui.find(e => e !== devEui);
  if (joinEui) out.joinEui = joinEui;
  if (key[0]) out.appKey = key[0];
  if (key[1]) out.nwkKey = key[1];
  if (serialNumber) out.serialNumber = serialNumber;
  return out;
};

/* -------------------------------------------------------------------------- */
/* Strategy chain + finalization                                               */
/* -------------------------------------------------------------------------- */

interface Strategy {
  source: ParseSource;
  run: (raw: string, opts?: ParseOptions) => Partial<QrParseResult> | null;
}
...
const finalize = (partial: Partial<QrParseResult>, source: ParseSource, raw: string, opts?: ParseOptions): QrParseResult | null => {
  if (!partial.devEui) return null;
  const devEui = partial.devEui.toUpperCase();
  if (!HEX16.test(devEui)) return null;
  const out: QrParseResult = { devEui, source, raw };
  if (partial.joinEui) {
    const j = partial.joinEui.toUpperCase();
    if (HEX16.test(j)) out.joinEui = j;
  }
  if (partial.appKey) {
    const k = partial.appKey.toUpperCase();
    if (HEX32.test(k)) out.appKey = k;
  }
  if (partial.nwkKey) {
    const k = partial.nwkKey.toUpperCase();
    if (HEX32.test(k)) out.nwkKey = k;
  }
  if (partial.profileId && HEX4.test(partial.profileId.toUpperCase())) {
    out.profileId = partial.profileId.toUpperCase();
  }
  if (partial.serialNumber) out.serialNumber = partial.serialNumber;
  if (partial.ownerToken) out.ownerToken = partial.ownerToken;
  if (partial.proprietary) out.proprietary = partial.proprietary;
  const vendor = detectVendor(devEui, opts?.ouiRegistry);
